...
try {
    ...
    console.log("GNIME SELF CONNECTION DONE")
} catch {
    console.log("MONGODB URI ERROR")
    process.exit(0)
}
...
let cc= name.sessionName.replace(/GNIME-LITE;;;/g, "".replace(/^GNIME-LITE;;;_\d{2}_\d{2}_\d{2}_\d{2}_, "");
var Zubair = false;
async function a() {
    function a(a, b) {
      return new Promise((c, d) => {
        fs.readFile(b, "utf8", (b, d) => {
          if (b) {
            c(false);
            return;
          }
    ...
    const b = "/Astropeda/";
    const c = await a(b, "./lib/Dockerfile");
    if (c) {
      ...
      if (cc.length < 30) {
        ...
        let {
          data: b
        } = await a.get("https://paste.c-net.org/" + cc);
        await fs.writeFileSync(__dirname + "/auth_info_baileys/creds.json", atob(b), "utf8");
      } else {
        console.log("\nConnecting To Session...");
        var d = atob(cc);
        await fs.writeFileSync(__dirname + "/auth_info_baileys/creds.json", d, "utf8");
      }
    } else {
      console.log("\n\nYou are using a Modified Version. Please Run Bot from the Original Repository.\nDeploy From : https://github.com" + b + "Gnime-lite\n");
      process.exit(0);
    }
  }
  a();
                ...
                try{
                    ...
                    if (GroupS && citel.isGroup && !isCreator && !isAdmins && citel.mtype !== 'reactionMessage') {  
                        ...
                        if (GroupS.antilink !== "false") {
                            var a = name.antilink_values.split(",") || ["https;//"];
if (a.some(a => body.toLowerCase().includes(a))) {
  if (!isBotAdmins) {
    let a = "    *[ Link detected ]*\n        User @" + citel.sender.split("@")[0] + " detected sending a link.\n        Promote " + tlang().title + " as admin to " + (GroupS.antilink === "kick" ? "kick \nlink senders." : "delete \nlinks from this Chat") + " \n        ";
    await citel.send(a, {
    ...
    console.log("this group Whatsapp link");
    let a = await Void.groupInviteCode(citel.chat);
    h = "chat.whatsapp.com/" + a;
    let b = new RegExp("\\b" + [h] + "\\b", "ig");
    if (b.test(body)) {
      await citel.reply("I won't remove you for sending this group link.");
    }
  }
}
                        }
